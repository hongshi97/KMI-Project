...
# maxdepth = 8 일 때 extractRules 결과 18426개의 rule 발견

ruleExec <- unique(ruleExec)
# unique() 함수 적용한 결과 18275개의 행 존재
...
presentRules(ruleMetric_result, colnames(X))

# getFreqPattern(ruleMetric, minsup = 0.01, minconf = 0.5, minlen = 1, maxlen = 8)

# pruneRule()을 한 결과를 getFreqPattern()함수에 input으로 넣기
freqPattern <- getFreqPattern(temp_rule, minsup = 0.01, minconf = 0.5, minlen = 2, maxlen = 6)
