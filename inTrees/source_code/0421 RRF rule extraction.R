...
df$AGE <- as.numeric(df$AGE)   # factor형 한 후에 다시 numeric으로 바꾸고 난 후 확인해보니 AGE = 1은 만 12살을 의미, AGE = 7은 만 18살을 의미
is.numeric(df$AGE)
...
#ruleMetric2 <- data.frame(ruleMetric)
#ruleMetric2$freq <- as.numeric(ruleMetric2$freq)
#ruleMetric2$err <- as.numeric(ruleMetric2$err)

#ruleMetric2$Acc <- sapply(ruleMetric2['err'], function(x) { 1 - x})
#ruleMetric2$new_standard <- ruleMetric2$freq * ruleMetric2$Acc      # freq * ( 1 - err )로 새로 기준 만들기

#presentRules(ruleMetric2, colnames(X))


# new_standard (freq * ( 1- err)) 값 내림차순으로 결과 확인

#ruleMetric2[order(ruleMetric2$new_standard, decreasing = TRUE),]
...
rule_after_prune <- data.frame(rule_after_prune)

rule_after_prune$freq <- as.numeric(rule_after_prune$freq)
rule_after_prune$err <- as.numeric(rule_after_prune$err)

rule_after_prune$Acc <- sapply(rule_after_prune['err'], function(x) { 1 - x})
rule_after_prune$new_standard <- rule_after_prune$freq * rule_after_prune$Acc      # freq * ( 1 - err )로 새로 기준 만들기

rule_after_prune <- rule_after_prune[order(rule_after_prune$new_standard, decreasing = TRUE),]

rule_pred_0 <- rule_after_prune %>% filter(pred == 0)
rule_pred_1 <- rule_after_prune %>% filter(pred == 1)

write.csv(rule_after_prune, '0415_RF_rules_after_prune.csv')
write.csv(rule_pred_0, '0415_rule_pred_0.csv')
write.csv(rule_pred_1, '0415_rule_pred_1.csv')
"

# pruneRule()을 한 결과를 getFreqPattern()함수에 input으로 넣기
freqPattern <- getFreqPattern(temp_rule, minsup = 0.01, minconf = 0.7, minlen = 1, maxlen = 8) 
...
write.csv(freqPattern, '0421_RF_rules_maxdep_8,minconf_0.7.csv')


#write.csv(freqPattern, '0421_RF_rules_maxdep_8,minconf_0.8.csv')
#write.csv(freqPattern, '0421_RF_rules_maxdep_8,minconf_0.75.csv')
