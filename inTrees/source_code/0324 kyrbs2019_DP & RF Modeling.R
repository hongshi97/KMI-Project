...
set.seed(10)
rf.fit = randomForest(BMI ~., data=train, mtry = floor(sqrt(157)), importance = T)  # floor(sqrt(157)) = 12         floor() = 내림 함수
rf.fit

test_x <- test %>% select(-BMI)
test_y <- test$BMI

y_pred = predict(rf.fit, test_x)

cfm = confusionMatrix(data = y_pred, reference = test_y, positive = '1')  # data = 예측값, reference = 실제값
cfm

library(pROC)
y_pred2 <- ordered(y_pred, levels = c(0,1))
test_y2 <- ordered(test_y, levels = c(0,1))

auc(roc(test_y2, y_pred2))

importance(rf.fit)
varImpPlot(rf.fit)

a <- data.frame(importance(rf.fit))
names(a)
b <- a[order(-a$MeanDecreaseAccuracy),] %>% select(MeanDecreaseAccuracy)
head(b,37)
row.names(b)
